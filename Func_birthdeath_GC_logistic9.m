function Func_birthdeath_GC_logistic9(BindFlag,paramFlag,NAg,IC1,IC2,ClusterSize,A,CT,ProtF,OnTargetRCF,OffTargetRCF,FitAdv,ProtAg,id,memnumber,Corr12,Multiplier,Mutrate)
rng(1000)
Agcutoff=0;
% AllClones = [1:1:228];
expon=1;
...
%OnTargetClonesNPHA = [11 13 14 128 129 152 153]; % NPHA
...
%     TExpansionDecay = TMemoryGrowth;
    ...
    OutSideCellsArrSp = MergeCells(OutSideCellsArrSp,CellsArrMemorySpProfLeft,Nsp); %determines if ratio mem only.
%OutSideCellsArrSp =CellsArrMemorySpProfLeft; %if you only want memory B
%cells to seed future GCs, use this command instead of above.
...
%   save([flag_str,'_runs',num2str(runNum),'_mem',num2str(memnumber),'_CorrVV',num2str(Corr12),'mu=',num2str(FitAdv),'w=',num2str(2*Multiplier),'mut=',num2str(mutrate),'.mat'],'lambdaSim','AgCapMeanSim','AgCapSTDSim','IC1tArrAvg','IC2tArrAvg','EAgAbSim','EAgAbExitSim','AgCapExitSim',...
%         'q1Sim','q2Sim','AgCapPopSim','PopulationTrack','BCR_s','EAbAg','EAgMem','wcv','ExitProb','ICTypeNums','AgICprop','runNum',...
%         'Nsp','T0','T1','T2','T3','Nmax','k','BirthNumSim','MutDistSim','SeedingCellsSim','OutEngsCellSim',...
%         'BindFlag','paramFlag','NAg','ClusterSize','A','CT','id','OnTargetRCF','OffTargetRCF','FitAdv','bOff','bOn','COff','COn','-v7.3');
    save(['eng',flag_str,'_runs',num2str(runNum),'_mem',num2str(memnumber),'_CorrVV',num2str(Corr12),'mu=',num2str(FitAdv),'w=',num2str(2*Multiplier),'mut=',num2str(mutrate),'.mat'],'OutEngsCellSim');
