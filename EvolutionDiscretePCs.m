...
function EvolutionDiscretePCs(V0,BC0,Kab,PCspf,PCgc,Rg,WeakMem)
...
% RsArray=[2:-.04:0]*r;
% RsArray=[.7:.05:1.3]*r+0.6;
%  RsArray=[.7 1 1.3]*r+0.6;
 RsArray=[.9 :.1 :1.2]*r+0.6;
RgArray=[1.4:.1:3.5];
%  RgArray=[.7 1 1.3]*r;
% RsArray=[2,4];
% RgArray=[2,4];
...
Flagstr=['FunctionEv_V0',num2str(V0),'_BC0',num2str(BC0),'_Kab',num2str(Kab),'_Ps',num2str(PCspf),'_Pg',num2str(PCgc),'PCsLeave'];
...
k1BC=BCs.*fitness*Rs/(sum(BCs.*fitness)/sum(BCs))-PCspf*BCs;
k2BC=(BCs+k1BC*dt*.5).*fitness*Rs/(sum((BCs+k1BC*dt*.5).*fitness)/sum((BCs+k1BC*dt*.5)))-PCspf*(BCs+k1BC*dt*.5);
k3BC=(BCs+k2BC*dt*.5).*fitness*Rs/(sum((BCs+k2BC*dt*.5).*fitness)/sum((BCs+k2BC*dt*.5)))-PCspf*(BCs+k2BC*dt*.5);
k4BC=(BCs+k3BC*dt).*fitness*Rs/(sum((BCs+k3BC*dt).*fitness)/sum((BCs+k3BC*dt)))-PCspf*(BCs+k3BC*dt);
