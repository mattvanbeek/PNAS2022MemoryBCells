function Res = CompetGC(dt,T,NspArr,C,CellsArrSpExit,CellsArrSp,Nsp,ExitProb,cellNumDyn,AgCapMean,AgCapSTD,lambdaPop,AgCapPop,ICArrt)
global ICArr;
MuFlag=1;
...
% [lambdaMeanAll AgCapMeanAll CTMeanAll] = LambdaMeanFuncAgCapHABoost(CellsArrSp,NspArr);
for t=0:dt:T
...
%         Countq = Countq+1;
        ...
        E= [E,CellsArrSp{1,n}(j,1).BCR_s.aff.EAbAg(2)];
            end
        end
        EBindMean(C,1)=mean(E);
        EBindMedian(C,1)=median(E);
        EBindVar(C,1)=var(E);
        for n=NspArr
